"""For demo purposes only -- useful if we want to sample from other larger datasets
(like Sentinel 2) that we can't download the entire geoparquet version of.
"""
...
NUM_PROCESSES = 12
BOUNDS = [-124.85, 24.40, -66.89, 49.38]  # USA bounds generated by Copilot
...
def call_get_naip_around_point_time(idx):
    while True:
        lon = random.random() * WIDTH + BOUNDS[0]
        lat = random.random() * HEIGHT + BOUNDS[1]
        row = None
        try:
            row = get_naip_around_point_time(
                lat, lon, date_range=DATE_RANGE, output_fn=f"data/sample_{idx}.tif"
            )
...
def main():
    print(f"Running parallel NAIP sampler with {NUM_PROCESSES} processes")
    with multiprocessing.Pool(processes=NUM_PROCESSES) as pool:
        rows = list(
            tqdm(
                pool.imap(call_get_naip_around_point_time, range(NUM_POINTS)),
                total=NUM_POINTS,
            )
        )
